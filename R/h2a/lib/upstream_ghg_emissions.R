...
#
import::from("helpers.R", FIRST, SECOND, THIRD, get, .directory = here(h2a))
import::from("ref_tables.R", upstream_energy_and_emissions, .directory = here(h2a))
calculate_ghg_emission_for_feedstock_and_ghg_column <- function(column_name, feedstock, energy_input) {
    ...
    return(energy_input * get(get(upstream_energy_and_emissions, column_name), get(feedstock, 'name')))
}
iterate_over_ghg_columns_for_feedstock <- function(feedstock, energy_input, greenhouse_gas_column_names) {
    ...
    return(lapply(greenhouse_gas_column_names, function(column_name) calculate_ghg_emission_for_feedstock_and_ghg_column(column_name, feedstock, energy_input)))
}
