...
#
import::here(FIRST, YEAR_1, get, sum_list, .from = "helpers.R", .directory = here("h2a"))
access <- function(i, table, row) {
    #'Safely access a value in a table
    if (FIRST + row - i >= YEAR_1 && FIRST + row - i < length(get(table, i)) + FIRST) {
        return(get(get(table, i), FIRST + row - i))
    }
    else {
        return(0)
    }
...
calculate_depreciation_charge <- function(year, recovery_index_range, depreciation_calculation_table) {
    ...
    return(sum_list(diagonal(depreciation_calculation_table, year, recovery_index_range)))
}
...
get_depr_charge <- function(charge, i, total_remaining_depreciation_charges, anal_period, construct) {
    ...
    if (i - FIRST == anal_period + construct - 1) {
