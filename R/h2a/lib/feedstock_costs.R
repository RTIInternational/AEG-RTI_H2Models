...
get_total_feedstock_costs <- function(operation_range, feedstock_price_df, inflation_price_increase_factors, start_time, plant_output_kg_per_year, percnt_var) {
    ...
    return(mapply(function(year) list(calculate_feedstock_cost_for_year(get(operation_range, year), sum_list(feedstock_prices_from_df(feedstock_price_df, year)), get(inflation_price_increase_factors, year), start_time, plant_output_kg_per_year, percnt_var)), range(length(operation_range))) )
}
