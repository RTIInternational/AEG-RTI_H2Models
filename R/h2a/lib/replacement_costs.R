...
calculate_replacement_cost <- function(year, inflation_price_increase_factor, replacement_costs_for_years, replace_factor, direct_cap, depr_cap, ref_year, startup_year, inflation_rate) {
    if (year < 0) {
        ...
        return(-inflation_price_increase_factor * (eval(parse(text = to_str(get(replacement_costs_for_years, to_str(year), 0)))) + (replace_factor * depr_cap)) * ((1 + inflation_rate) ** (startup_year - ref_year)))
    }
}
