...
#
from h2a.helpers import FIRST, TRUE, concat, get, get_cell, split, to_num
...
def get_feedstock_price_for_year(year, feedstock, INFLATION_FACTOR):
    ...
    return (
        get_cell(get_aeo(get(feedstock, "price_table")), year, get(feedstock, "name"))
...
def get_feedstock_price_for_startup_year(
    ...
    return (
        get_cell(
            get_aeo(get(feedstock, "price_table")), year, get(feedstock, "startup_year")
        )
        * to_num(get(feedstock, "usage"))
        * conversion_factor(
            concat(get(split(get(feedstock, "units"), "/"), FIRST), "_to_$/GJ")
