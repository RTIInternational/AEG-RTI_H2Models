...
#
from h2a.helpers import FIRST, TRUE, YEAR_1, get, length, sum_list


def access(i, table, row):
    """Safely access a value in a table"""
    if FIRST + row - i >= YEAR_1 and FIRST + row - i < length(get(table, i)) + FIRST:
...
def calculate_depreciation_charge(
    ...
    return sum_list(
        diagonal(depreciation_calculation_table, year, recovery_index_range)
    )
